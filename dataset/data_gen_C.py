#!/usr/bin/env python
# coding: utf-8
# Type-C: internal bricks: False; mass: equal; interactions: 3; time: 5s; number: 50
...
# Global Variables
SIMULATION_TIME = 6 # Total simulation time in seconds
    ...
    for _ in range(TOTAL_BALLS // 2):
        ...
        if connection_type == "spring":
            ...
            spring = pymunk.DampedSpring(
                balls[-2].body, balls[-1].body, (0, 0), (0, 0), rest_length, SPRING_STIFFNESS, 0
            )
            space.add(spring)
            connections.append(("spring", spring))
        elif connection_type == "rigid_rod":
            rod = pymunk.PinJoint(balls[-2].body, balls[-1].body, (0, 0), (0, 0))
            space.add(rod)
            connections.append(("rigid_rod", rod))
        elif connection_type == "rope":
            rope = pymunk.SlideJoint(
                balls[-2].body, balls[-1].body, (0, 0), (0, 0), 0, ROPE_MAX_LENGTH
            )
            space.add(rope)
...
if __name__ == "__main__":
    for _ in range(1):
        VIDEO_FILENAME = f"type_C_{datetime.now().strftime('%Y%m%d_%H%M%S')}.mp4"
